...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    # propagate early stopping settings if provided by config
    if hasattr(cfg, 'early_stop') and isinstance(cfg.early_stop, dict):
        trainer.early_stop_patience = int(cfg.early_stop.get('patience', trainer.early_stop_patience))
        trainer.early_stop_min_delta = float(cfg.early_stop.get('min_delta', trainer.early_stop_min_delta))
    ...
    trainer.run(data_loaders, cfg.workflow, cfg.total_epochs, cfg.fade_epoch, local_rank=cfg.local_rank)
