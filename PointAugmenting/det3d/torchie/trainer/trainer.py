...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Defaults; can be overridden by caller after construction
        self.early_stop_patience = 5
        self.early_stop_min_delta = 0.0
        # Loss history for visualization
        self.train_loss_history = []
        self.val_loss_history = []
        self.last_val_map = None
    ...
    def save_best_checkpoint(self, out_dir, meta=None):
        """Save a checkpoint named 'best.pth' without optimizer to be lightweight."""
        filename = "best.pth"
        filepath = osp.join(out_dir, filename)
        save_checkpoint(self.model, filepath, optimizer=None, meta=meta)
    ...
    def train(self, data_loader, epoch, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            # accumulate scalar total loss for plotting
            if "loss" in outputs and hasattr(outputs["loss"], "item"):
                try:
        ...
        # Record average train loss this epoch
        if epoch_loss_count > 0:
            self.train_loss_history.append(epoch_loss_sum / max(1, epoch_loss_count))
        else:
    ...
    def val(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                # Also compute validation loss for curve visualization
                try:
                    loss_out = self.batch_processor(
                        self.model, data_batch, train_mode=True, **kwargs
                    )
                    if isinstance(loss_out, dict) and "loss" in loss_out:
                        val_loss_sum += float(loss_out["loss"].item())
                        val_loss_count += 1
                except Exception:
                    # non-fatal: some models may not support val loss
                    pass
        ...
        # Record average val loss
        if val_loss_count > 0:
            self.val_loss_history.append(val_loss_sum / max(1, val_loss_count))
        else:
            self.val_loss_history.append(float("nan"))

        # Track best mAP@0.5 and save best checkpoint
        cur_map = None
        try:
            cur_map = float(result_dict["results"].get("mAP@0.5", None))
        except Exception:
            cur_map = None
        self.last_val_map = cur_map
        if cur_map is not None and self.rank == 0:
            improved = (cur_map - self.best_map) > self.early_stop_min_delta
            if improved:
                self.best_map = cur_map
                self.best_epoch = self.epoch
                self.no_improve_epochs = 0
                self.logger.info(f"New best mAP@0.5={cur_map:.4f} at epoch {self.epoch+1}, saving best.pth")
                meta = dict(epoch=self.epoch + 1, iter=self.iter, metric="mAP@0.5", score=cur_map)
                self.save_best_checkpoint(self.work_dir, meta=meta)
            else:
    ...
    def run(self, data_loaders, workflow, max_epochs, fade_epoch=20, **kwargs):
        """ Start running.
        ...
        while self.epoch < max_epochs:
            if self.epoch >= fade_epoch:
                data_loaders[0].dataset.pipeline.transforms[2].db_sampler = None
            for i, flow in enumerate(workflow):
                ...
                for _ in range(epochs):
                    if mode == "train" and self.epoch >= max_epochs:
                        ...
                        # Early stopping check after each val epoch (rank 0 only)
                        if self.rank == 0 and self.last_val_map is not None:
                            if self.no_improve_epochs >= self.early_stop_patience:
                                self.logger.info(
                                    f"Early stopping triggered (no improvement in {self.early_stop_patience} epochs)."
                                )
                                # Plot curves before exiting
                                try:
    ...
    def _plot_loss_curves(self):
        """Plot train/val loss curves to work_dir/loss_curve.png"""
        if self.rank != 0:
            return
        try:
            import matplotlib
            ...
            self.logger.warning(f"Matplotlib not available for plotting: {e}")
            return
        ...
        out_path = osp.join(self.work_dir if self.work_dir else ".", "loss_curve.png")
        plt.tight_layout()
