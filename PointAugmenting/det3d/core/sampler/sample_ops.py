            ...
            for info in sampled:
                try:
                    ...
                    if raw.size % num_point_features == 0:
                        s_points = raw.reshape(-1, num_point_features)
                    else:
                        # try fallback to 5-dim and pad zeros for extras
                        if raw.size % 5 == 0 and num_point_features >= 5:
                            base = raw.reshape(-1, 5)
                            if num_point_features > 5:
                                padded = np.zeros((base.shape[0], num_point_features), dtype=np.float32)
                                padded[:, :5] = base
                                s_points = padded
                            else:
                                s_points = base
                        else:
                            raise ValueError("GT DB points feature mismatch: got {} floats, expecting {} per point".format(raw.size, num_point_features))
        ...
        if len(sampled) > 0:
            ...
            for i, info in enumerate(sampled):
                try:
                    ...
                    # robust reshape: support legacy 5-dim db and 8-dim cross-modal db
                    if raw.size % 8 == 0:
                        ...
                        raise ValueError("GT DB points feature mismatch: {} floats".format(raw.size))
                    s_points = pts8[:, selected_feature > 0.]
            ...
            # 如果没有任何可用的点（例如图像读失败或补丁为空），直接放弃本次采样
