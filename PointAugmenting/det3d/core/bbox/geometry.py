...
def points_count_convex_polygon_3d_jit(points, polygon_surfaces, num_surfaces=None):
    ...
    Returns:
        [num_polygon] array.
    """
    # Ensure dtype/contiguity for numba
    points = np.ascontiguousarray(points, dtype=np.float32)
    surfaces = np.ascontiguousarray(polygon_surfaces[:, :, :3, :], dtype=np.float32)
    max_num_surfaces, max_num_points_of_surface = surfaces.shape[1:3]
    num_points = points.shape[0]
    num_polygons = surfaces.shape[0]
    if num_surfaces is None:
        num_surfaces = np.full((num_polygons,), 9999999, dtype=np.int64)
    # Use NumPy version to avoid Numba unboxing issues
    normal_vec, d = surface_equ_3d(surfaces)
    normal_vec = np.ascontiguousarray(normal_vec, dtype=np.float32)
    d = np.ascontiguousarray(d, dtype=np.float32)
    # normal_vec: [num_polygon, max_num_surfaces, 3]
    # d: [num_polygon, max_num_surfaces]
    return _points_count_convex_polygon_3d_jit(
        points, surfaces, normal_vec, d, num_surfaces
    )
    ...
    for i in range(num_points):
        ...
        for j in range(num_polygons):
            ...
            for k in range(num_points_of_polygon):
                prev = k - 1
                if prev < 0:
                    prev = num_points_of_polygon - 1
                ex = polygon[j, k, 0] - polygon[j, prev, 0]
                ey = polygon[j, k, 1] - polygon[j, prev, 1]
                vx = polygon[j, k, 0] - px
                vy = polygon[j, k, 1] - py
                cross = ey * vx - ex * vy
                if clockwise:
...
def _points_in_convex_polygon_3d_jit_v2(points, surfaces):
    """Variant that internally computes plane equations. Not used by default."""
    num_polygons = surfaces.shape[0]
    max_num_surfaces = surfaces.shape[1]
    num_points = points.shape[0]
    normal_vec, d = surface_equ_3d_jitv2(surfaces)
    ret = np.ones((num_points, num_polygons), dtype=np.bool_)
    # use all surfaces by default
    num_surfaces = np.full((num_polygons,), 9999999, dtype=np.int64)
    sign = 0.0
    for i in range(num_points):
        for j in range(num_polygons):
