...
class DLA34(nn.Module):
    def __init__(self, levels=[1, 1, 1, 2, 2, 1], channels=[16, 32, 64, 128, 256, 512], num_classes=1000,
                 block=BasicBlock, residual_root=False, linear_root=False):
...
def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
    ...
    if name.endswith('.pth'):
        # 本地加载
        model_path = data + name  # e.g. './pretrained_model/dla34.pth'
        print(f"=> loading pretrained model from {model_path}")
        checkpoint = torch.load(model_path, map_location="cpu")

        # 兼容两种情况：有/没有 'state_dict'
        ...
        model_weights = model_zoo.load_url(model_url, map_location="cpu")
...
class DLAUp(nn.Module):
    def __init__(self, startp, channels, scales, in_channels=None,
                 node_type=(DeformConv, DeformConv)):
...
class DLASeg(nn.Module):
    def __init__(self):
        ...
        model_path = "./pretrained_model/dla34.pth"
        checkpoint = torch.load(model_path, map_location="cpu")
        if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
             model_weights = checkpoint["state_dict"]
        else:
