import numpy as np
try:
    # Prefer spconv 2.x API
    from spconv.pytorch import (
        SparseConv3d, SubMConv3d, SparseConvTensor, SparseSequential, SparseModule
    )
    import spconv.pytorch as spconv
except Exception:  # pragma: no cover - fallback for older spconv
...
def _replace_feature(x, new_feat):
    """Compat helper: spconv 2.x forbids direct assignment to .features.
    Returns a SparseConvTensor with replaced features. Falls back to direct
    assignment for older spconv versions.
    """
    if hasattr(x, "replace_feature"):
        return x.replace_feature(new_feat)
    x.features = new_feat
    return x

def conv3x3(in_planes, out_planes, stride=1, indice_key=None, bias=True):
    """3x3 submanifold convolution with padding"""
    return SubMConv3d(
        in_planes,
        out_planes,
        kernel_size=3,
...
class SpMiddleResNetFHD(nn.Module):
    def __init__(
        ...
        # When densified and flattened along D, expected BEV channels are 256 (128*C with D≈2).
        # However, for some datasets/shapes D can become 1, yielding 128 channels and causing
        # a mismatch with RPN configured for 256 input channels. Add a lightweight adapter
        # to map 128->256 when needed.
        self._bev_channel_adapter = nn.Sequential(
            nn.Conv2d(128, 256, kernel_size=1, bias=False),
            build_norm_layer(dict(type="BN", eps=1e-3, momentum=0.01), 256)[1],
            nn.ReLU(inplace=True),
        )

    def forward(self, voxel_features, coors, batch_size, input_shape, bev_sparse=None):
        ...
        # Channel adapt if BEV channels are 128 to satisfy downstream RPN expecting 256.
        if ret.shape[1] == 128:
            ret = self._bev_channel_adapter(ret)
