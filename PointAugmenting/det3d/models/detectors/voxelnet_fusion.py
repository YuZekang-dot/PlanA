...
class VoxelNetFusion(SingleStageDetector):
    ...
    def get_img_feat(self, img, pts_uv, voxels_valid):
        ...
        with torch.no_grad():
            # image shape: B, Cams, 3, H, W
            cams = img.shape[1]
            img = img.view(-1, 3, img.shape[3], img.shape[4])  # B*Cams, 3, H, W
            img_feat = self.img_backbone(img)
            img_feat = img_feat.view(batch_size, cams, -1, img_feat.shape[2], img_feat.shape[3]).transpose(2, 1)
            # print('img_feat', img_feat.shape)

            # Explicitly set align_corners to silence warning; default since PyTorch 1.3 is False
            voxel_img_feat = F.grid_sample(
                img_feat, pts_uv, mode='bilinear', padding_mode='zeros', align_corners=False
            )
