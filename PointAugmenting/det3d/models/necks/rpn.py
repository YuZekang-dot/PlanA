...
class RPN(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(ups) > 0:
            # Ensure all upsampled feature maps have the same spatial size before concat.
            # Sometimes ConvTranspose2d / Conv2d with differing strides produce off-by-one sizes
            # (e.g. 47 vs 48). Resize others to the first tensor's HxW using interpolation.
            target_h = ups[0].shape[2]
            target_w = ups[0].shape[3]
            aligned_ups = []
            for feat in ups:
                if feat.shape[2] != target_h or feat.shape[3] != target_w:
                    # Use bilinear interpolation for spatial resizing of 2D feature maps
                    feat = F.interpolate(feat, size=(target_h, target_w), mode='bilinear', align_corners=False)
                aligned_ups.append(feat)
            x = torch.cat(aligned_ups, dim=1)
        return x
