...
class CenterHead(nn.Module):
    def __init__(
        ...
        # Prepare class weights per task if provided
        self.class_weights_per_task = None
        if class_weights is not None:
            # If single list and single task, use directly. If multiple tasks, try to split by class count.
            if isinstance(class_weights, (list, tuple)):
                if len(num_classes) == 1:
                    self.class_weights_per_task = [list(class_weights)]
                else:
                    # Flattened weights provided; slice per task
                    flat = list(class_weights)
                    self.class_weights_per_task = []
                    start = 0
                    for n in num_classes:
                        self.class_weights_per_task.append(flat[start:start+n])
                        start += n
            else:
                # Tensor provided, split accordingly
                flat = class_weights.view(-1)
                self.class_weights_per_task = []
                start = 0
                for n in num_classes:
                    self.class_weights_per_task.append(flat[start:start+n])
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # Optional class weights for this task
            cw = None
            if self.class_weights_per_task is not None and task_id < len(self.class_weights_per_task):
                cw = self.class_weights_per_task[task_id]
            ...
            # Support waymo / nuscenes / kitti. For kitti we typically don't use velocity.
            if self.dataset in ['waymo', 'nuscenes', 'kitti']:
            ...
            # Align code_weights length with the regression code dimension to avoid size mismatch
            # anno_box layout (no vel): [reg(2), height(1), dim(3), rot(2)] => 8 dims
            # anno_box layout (with vel): [reg(2), height(1), dim(3), vel(2), rot(2)] => 10 dims
            expected_len = box_loss.shape[0] if box_loss.dim() > 0 else len(self.code_weights)
            cw_list = list(self.code_weights)
            if len(cw_list) != expected_len:
                if len(cw_list) > expected_len:
                    cw_list = cw_list[:expected_len]
                else:
                    cw_list = cw_list + [1.0] * (expected_len - len(cw_list))
            loc_loss = (box_loss * box_loss.new_tensor(cw_list)).sum()
