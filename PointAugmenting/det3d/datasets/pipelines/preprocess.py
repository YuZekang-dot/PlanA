...
class Preprocess(object):
    ...
    def __call__(self, res, info):
        ...
        elif res["type"] in ["NuScenesDataset", "KittiDataset"]:
            points = res["lidar"]["combined"]
        else:
        ...
        if self.mode == "train":
            ...
            # Convert KITTI camera-frame boxes to lidar frame and pad velocity zeros
            if res["type"] == "KittiDataset":
                def cam_boxes_to_lidar(boxes_cam, calib):
                    # boxes_cam: [N,7] = [x,y,z,l,w,h,ry] in camera rect frame (bottom center)
                    ...
                    # cam_rect = R0 @ Tr @ velo; thus velo = Tr^{-1} @ R0^{-1} @ cam_rect
                    ...
                    # heading vector in camera: rotate [0,0,1] by ry around Y
                    ...
                    l = dims_lwh[:, 0]
                    h = dims_lwh[:, 2]
                    boxes_lidar7 = np.stack([centers_velo[:, 0], centers_velo[:, 1], centers_velo[:, 2], w, l, h, yaw_lidar], axis=1)
                    # pad vx,vy zeros and place rot at index 8
                    zeros = np.zeros((boxes_lidar7.shape[0], 2), dtype=np.float32)
                    boxes_lidar9 = np.concatenate([boxes_lidar7[:, :6], zeros, boxes_lidar7[:, 6:7]], axis=1)
                    return boxes_lidar9.astype(np.float32)

                gt_boxes_lidar = cam_boxes_to_lidar(anno_dict["boxes"], res["calib"]).astype(np.float32)
            else:
                gt_boxes_lidar = anno_dict["boxes"].astype(np.float32)

            gt_dict = {
                "gt_boxes": gt_boxes_lidar,
                "gt_names": np.array(anno_dict["names"]).reshape(-1),
                # some datasets don't provide frustums; keep a placeholder
                "gt_frustums": anno_dict.get("frustums", np.zeros((gt_boxes_lidar.shape[0], 8, 3), dtype=np.float32)),
            }
        ...
        if self.mode == "train" and not self.no_augmentation:
            ...
            if self.db_sampler:
                ...
                # 统一采样8维原始点特征 [x,y,z,D,P,R,A,E]; 图像UV特征若需要会在后续由投影再追加
                selected_feature = np.ones([8], dtype=np.float32)
                # provide default group ids when not available to satisfy group sampling
                default_group_ids = np.arange(gt_dict["gt_boxes"].shape[0], dtype=np.int64)
                sampled_dict = self.db_sampler.sample_all_v2(
                    ...
                    random_crop=False,
                    revise_calib=True,
                    gt_group_ids=default_group_ids,
                    calib=calib,
...
class AssignLabel(object):
    ...
    def __call__(self, res, info):
        ...
        # Use ceil division so that label heatmap matches network output resolution
        # (backbone/neck may keep an extra cell due to padding), e.g., 300/8 -> 38 not 37
        feature_map_size = (grid_size[:2] + self.out_size_factor - 1) // self.out_size_factor
        example = {}
        ...
        if res["mode"] == "train":
            ...
            for idx, task in enumerate(self.tasks):
                ...
                if res['type'] in ['NuScenesDataset', 'KittiDataset']:
                    # [reg, hei, dim, vx, vy, rots, rotc]
                    anno_box = np.zeros((max_objs, 10), dtype=np.float32)
                elif res['type'] == 'WaymoDataset':
                    anno_box = np.zeros((max_objs, 10), dtype=np.float32) 
                else:
                    raise NotImplementedError("Only Support Waymo/nuScene/KITTI now!")
                ...
                for k in range(num_objs):
                    ...
                    if w > 0 and l > 0:
                        ...
                        if res['type'] == 'NuScenesDataset': 
                            ...
                            # KITTI annotations may not include velocities; default to zeros
                            vx, vy = 0.0, 0.0
                            rot = gt_dict['gt_boxes'][idx][k][-1]
                            anno_box[new_idx] = np.concatenate(
                                (ct - (x, y), z, np.log(gt_dict['gt_boxes'][idx][k][3:6]),
