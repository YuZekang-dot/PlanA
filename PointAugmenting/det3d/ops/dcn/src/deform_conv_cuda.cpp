...
void shape_check(at::Tensor input, at::Tensor offset, at::Tensor *gradOutput,
  ...
  TORCH_CHECK((weight.size(2) == kH && weight.size(3) == kW),
        ...
        "but got kH: ",
        kH, " kW: ", kW, " weight.size(2): ", weight.size(2),
        ", weight.size(3): ", weight.size(3));
  ...
  TORCH_CHECK(input.size(1) == nInputPlane,
              ...
              ", but got: ", input.size(1));

  TORCH_CHECK((inputHeight >= kH && inputWidth >= kW),
              "input image is smaller than kernel");

  TORCH_CHECK((offset.size(2) == outputHeight && offset.size(3) == outputWidth),
              "invalid spatial size of offset, expected height: ",
              outputHeight, " width: ", outputWidth, ", but got height: ",
              offset.size(2), " width: ", offset.size(3));
  ...
  if (gradOutput != NULL) {
    TORCH_CHECK(gradOutput->size(dimf) == nOutputPlane,
                "invalid number of gradOutput planes, expected: ",
                nOutputPlane, ", but got: ", gradOutput->size(dimf));

    TORCH_CHECK((gradOutput->size(dimh) == outputHeight &&
                 gradOutput->size(dimw) == outputWidth),
                "invalid size of gradOutput, expected height: ",
                outputHeight, " width: ", outputWidth,
                " , but got height: ", gradOutput->size(dimh),
                " width: ", gradOutput->size(dimw));
  }
}
...
int deform_conv_forward_cuda(at::Tensor input, at::Tensor weight,
  ...
  TORCH_CHECK((offset.size(0) == batchSize),
...
void modulated_deform_conv_cuda_forward(
  ...
  if (kernel_h_ != kernel_h || kernel_w_ != kernel_w)
    TORCH_CHECK(false,
                "Input shape and kernel shape wont match: (", kernel_h_,
                " x ", kernel_w_, " vs ", kernel_h, " x ", kernel_w, ").");
